...
public class GlasscutterMenu extends AbstractContainerMenu {
	private final ContainerLevelAccess access;
	private final DataSlot selectedRecipeIndex = DataSlot.standalone();
	private final Level level;
	private List<GlasscuttingRecipe> recipes = Lists.newArrayList();
	...
	public final Container container = new SimpleContainer(1) {
		public void setChanged() {
			super.setChanged();
			GlasscutterMenu.this.slotsChanged(this);
			GlasscutterMenu.this.slotUpdateListener.run();
		}
	};
	...
	public GlasscutterMenu(int id, Inventory inventory, final ContainerLevelAccess levelAccess) {
		...
		this.resultSlot = this.addSlot(new Slot(this.resultContainer, 1, 143, 33) {
			...
			public void onTake(Player player, ItemStack stack) {
				...
				GlasscutterMenu.this.resultContainer.awardUsedRecipes(player);
				ItemStack itemstack = GlasscutterMenu.this.inputSlot.remove(1);
				if (!itemstack.isEmpty()) {
					GlasscutterMenu.this.setupResultSlot();
				}

				levelAccess.execute((level, pos) -> {
					long l = level.getGameTime();
					if (GlasscutterMenu.this.lastSoundTime != l) {
						level.playSound((Player) null, pos, ModSounds.UI_GLASSCUTTER_TAKE_RESULT, SoundSource.BLOCKS, 1.0F, 1.0F);
						GlasscutterMenu.this.lastSoundTime = l;
					}
	...
	public List<GlasscuttingRecipe> getRecipes() {
		return this.recipes;
	}
	...
	void setupResultSlot() {
		if (!this.recipes.isEmpty() && this.isValidRecipeIndex(this.selectedRecipeIndex.get())) {
			GlasscuttingRecipe glasscuttingRecipe = this.recipes.get(this.selectedRecipeIndex.get());
			this.resultContainer.setRecipeUsed(glasscuttingRecipe);
			this.resultSlot.set(glasscuttingRecipe.assemble(this.container));
		} else {
			this.resultSlot.set(ItemStack.EMPTY);
		}
