...
public class WeaverContainer extends Container
{
   ...
   public final IInventory inputInventory = new Inventory(1) {
      ...
      public void markDirty() {
         super.markDirty();
         WeaverContainer.this.onCraftMatrixChanged(this);
         WeaverContainer.this.inventoryUpdateListener.run();
      }
   };
   private final CraftResultInventory inventory = new CraftResultInventory();

   public WeaverContainer(int windowIdIn, PlayerInventory playerInventoryIn) {
   ...
   public WeaverContainer(int windowIdIn, PlayerInventory playerInventoryIn, final IWorldPosCallable worldPosCallableIn) {
      ...
      this.outputInventorySlot = this.addSlot(new Slot(this.inventory, 1, 143, 33) {
         ...
         public ItemStack onTake(PlayerEntity thePlayer, ItemStack stack) {
            ItemStack itemstack = WeaverContainer.this.inputInventorySlot.decrStackSize(1);
            if (!itemstack.isEmpty()) {
               WeaverContainer.this.updateRecipeResultSlot();
            }
            ...
            worldPosCallableIn.consume((p_216954_1_, p_216954_2_) -> {
               ...
               if (WeaverContainer.this.lastOnTake != l) {
                  p_216954_1_.playSound((PlayerEntity)null, p_216954_2_, SoundEvents.UI_LOOM_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
                  WeaverContainer.this.lastOnTake = l;
               }
   ...
   @OnlyIn(Dist.CLIENT)
   public List<WeavingRecipe> getRecipeList() {
      return this.recipes;
   }
   ...
   public boolean canInteractWith(PlayerEntity playerIn) {
      return isWithinUsableDistance(this.worldPosCallable, playerIn, ModBlocks.WEAVER);
   }
   ...
   private void updateRecipeResultSlot() {
      if (!this.recipes.isEmpty()) {
         WeavingRecipe weavingrecipe = this.recipes.get(this.selectedRecipe.get());
         this.outputInventorySlot.putStack(weavingrecipe.getCraftingResult(this.inputInventory));
      } else {
         this.outputInventorySlot.putStack(ItemStack.EMPTY);
      }
